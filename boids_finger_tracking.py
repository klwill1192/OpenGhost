...
# keypress settings
SEND_KEY_ON_PINCH_RELEASE = True
KEY_TO_SEND = "f"
KEYPRESS_COOLDOWN_SECONDS = 0.25

# Set this to the title of your asciiquarium xterm window.
# Launch xterm with: xterm -T asciiquarium-tracker ...
TARGET_WINDOW_NAME = "asciiquarium-tracker"
...
class Boid:
    ...
    def apply_flocking_behaviors(
        ...
        # Add cursor attraction or repulsion force.
        if cursor_active:
...
def send_keypress() -> None:
    """Send the configured key using xdotool."""
    if not SEND_KEY_ON_PINCH_RELEASE:
        return
    ...
    try:
        ...
        print(f"Could not send keypress with xdotool: {exc}")
    ...
    try:
        while stop_tracking is None or not stop_tracking.is_set():
            ...
            if result.multi_hand_landmarks:
                ...
                thumb_near = (
                    thumb_dist_sq
                    <= THUMB_DISTANCE_THRESHOLD * THUMB_DISTANCE_THRESHOLD
                )
                ...
                with finger_lock:
                    if thumb_near:
                        ...
                        # Fire once when a pinch is released.
                        if pinch_was_active:
...
def draw():
    ...
    with finger_lock:
        cursor_x = finger_pos[0]
        cursor_y = finger_pos[1]
